...
@section('content')
    <div class="max-w-2xl space-y-5">
        <div class="bg-white rounded-xl shadow-sm p-6">
            <form method="POST" action="{{ auth()->user()->isAdmin() ? route('admin.karyawan.update', $employee) : route('manajer.karyawan.update', $employee) }}" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ auth()->user()->isAdmin() ? route('admin.karyawan.index') : route('manajer.karyawan.index') }}" class="px-5 py-2.5 rounded-xl bg-gray-100 text-gray-700 font-semibold">Batal</a>
                </div>
            </form>
        </div>
