...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody class="divide-y divide-gray-100">
                        ...
                        <td class="px-4 py-3">
                            ...
                            @if ($isAdmin && $req->status === 'pending')
                                <div class="flex justify-end gap-2">
                                    <form method="POST" action="{{ route('admin.lembur.verify', $req) }}">
                                        @csrf
                                    ...
                                    <form method="POST" action="{{ route('admin.lembur.approve', $req) }}">
                                        @csrf
                            ...
                            @elseif ($canFinal)
                                <div class="flex justify-end gap-2">
                                    <form method="POST" action="{{ $isAdmin ? route('admin.lembur.approve', $req) : route('manajer.lembur.approve', $req) }}">
                                        @csrf
                ...
                @empty
                    <tr><td colspan="7" class="px-4 py-8 text-center text-gray-400">Tidak ada pengajuan lembur.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    <div id="reject-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/50 p-6">
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6">
            <h3 class="font-bold text-gray-800 mb-4">Tolak Pengajuan Lembur</h3>
...
@push('scripts')
    <script>
        function openReject(id) {
            document.getElementById('reject-form').action = "{{ auth()->user()->isAdmin() ? route('admin.lembur.reject', ['lembur' => '__ID__']) : route('manajer.lembur.reject', ['lembur' => '__ID__']) }}".replace('__ID__', id);
